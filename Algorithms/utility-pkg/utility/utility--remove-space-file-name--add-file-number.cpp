...
std::string
createModifiedNameCPP(std::string pszFileName, std::string pszFileNum)
{
...
{
#ifndef ONLINE_JUDGE
    freopen((std::string{__FILE__}.substr(0, std::string{__FILE__}.find('.')) + "Error.txt").c_str(), "w", stderr);
#endif
    ...
    while (true)
    {
        ...
        // windows specific
        // tempFile.open("..\\" + modifiedFileName, std::ofstream::out);
        fs::path pathRelativePathToCreatedFiles = ("created-files");
        if (!fs::exists(pathRelativePathToCreatedFiles))
        {
            fs::create_directory(pathRelativePathToCreatedFiles);
        }
